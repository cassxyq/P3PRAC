...
resource "aws_security_group" "myprac-sg" {
  ...
  ingress {
    ...
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"] #哪些ip address range被允许access resource on port 22                       
  }
  ...
  tags = {
    Name    = "${var.env_prefix}-sg" #若新建要改下标签
    Project = "prac"
  }
}
...
data "aws_ami" "latest-amazon-linux-image" {
  ...
  filter {
    ...
    values = ["ubuntu/images/hvm-ssd/ubuntu-focal-20.04-amd64-server-*"] #要找的值是名字带这些的 *是任意
  }
  filter {
...
resource "aws_instance" "myprac-server" {
  ...
  vpc_security_group_ids = [aws_security_group.myprac-sg.id]
